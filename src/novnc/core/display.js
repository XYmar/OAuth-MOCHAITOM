...
Display.prototype = {
    ...
    viewportChangeSize: function(width, height) {
        ...
        if (vp.w !== width || vp.h !== height) {
            ...
            this._rescalexy(this._scalex,this._scaley);
        }
    },

    absX: function (x) {
        return x / this._scalex + this._viewportLoc.x;
    },

    absY: function (y) {
        return y / this._scaley + this._viewportLoc.y;
    },
    ...
    autoscale: function (containerWidth, containerHeight) {
        ...
        if (fbAspectRatio >= targetAspectRatio) {
            ...
            scaleRatio1 = containerHeight / vp.h;
        } else {
            scaleRatio = containerWidth / vp.w;
            scaleRatio1 = containerHeight / vp.h;
        }
        this._rescalexy(scaleRatio,scaleRatio1);
    },
    ...
    // 虚拟机界面宽高等比例缩放 start（mujr）
    _rescalexy: function (factor,factor1) {
        this._scale = factor;
        this._scalex = factor;
        this._scaley = factor1;
        var vp = this._viewportLoc;
        ...
        //                   ('NNNpx'), the canvas is scaled without clearing.
        var width = Math.round(factor * vp.w) + 'px';
        var height = Math.round(factor1 * vp.h) + 'px';
